...
import PrivateKeyModal from './setting/PrivateKeyModal';
...
function Customize({getOpenCustomizer, customizerOpen, navigation}) {
  ...
  const evmAccount = Accounts.find(account => account.hasOwnProperty('evm'));

  // Retrieve the mnemonic if the EVM account exists
  const evmMnemonic = evmAccount ? evmAccount.evm.mnemonic : '';
  console.log("-----------",evmMnemonic)
  ...
  return (
    <>
      ...
      <Actionsheet
        ...
        <Actionsheet.Content style={{backgroundColor: theme.screenBackgroud}}>
          <Box w="100%" h={550} px={4} justifyContent="center">
            ...
            <PrivateKeyModal privateKey={selectedAccountKey?.evm?.mnemonic} label="mnemonic " />
