...
export default function Trancactions({ navigation }) {
  ...
  const renderSolTransactions =  ({ item }) => {
    ...
    return (
      ...
      /> */}
      {net == 'evm' ? 
      <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center' , gap:80,marginTop:10}}> 
      <TouchableOpacity onPress={() => setShowPopup(true)}>
      <Text style={{color:theme.addButtonBorder}}>{t('date_filter')}</Text>
      </TouchableOpacity>
      <TouchableOpacity onPress={() => setTrx(trxData)}>
      <Text style={{color:theme.addButtonBorder}}>{t('clear_filter')}</Text>
      </TouchableOpacity>
      </View>
      :""}
            ...
            <View style={styles.container}>
                {Loader ? <MaroonSpinner /> :
                    <>
                     ...
                     <Text style={{color:theme.text}}>{t('no_transaction_found')}</Text>
                     </View>
                     :
                        <FlatList 
                          data={
