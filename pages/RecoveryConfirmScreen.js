...
const MnemonicComponent = ({ mnemonic, theme, hiddenIndices, setInputValues, inputValues }) => {
  ...
  <FlatList
    ...
    renderItem={({ item, index }) => (
      <View key={index} style={{ marginBottom: 5, width: '80%', marginRight: -87 }}>
        <TouchableOpacity onPress={() => handleWordPress(index)} style={{ borderWidth: 1, borderRadius: 5, paddingHorizontal: 5, borderColor: theme.wordBorder, width: '60%', flexDirection: 'row', alignItems: 'center', height: 40 }}>
          {hiddenIndices.includes(index) ? (
            <TextInput
              style={{ color: theme.text, flex: 1, textAlign:'center' }} 
