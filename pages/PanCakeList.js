...
const PanCakeList = ({ navigation }) => {
  ...
  <ScrollView style={[styles.MainWrapper, {backgroundColor: theme.screenBackgroud}]}>
    ...
    {Session.length === 0 ? (
      <View style={[styles.centeredContent, {backgroundColor: theme.screenBackgroud, alignItems:'center',justifyContent:'center'}]}>
        <Text style={[styles.noDataText, {color: theme.text,marginTop:'60%',paddingHorizontal:30,textAlign:'center'}]}>
        Connect your wallet with Dapps to see them listed here
        </Text>
        <SubmitBtn
                title='Connect your account'
                onPress={() => navigation.navigate('Scan')}
                containerStyle={{ paddingHorizontal:10, marginTop:20}}
              />
      </View>
    ) : (
